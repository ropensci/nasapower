...
# validate cell lon lat user provided values and return identifier -------------
.check_lonlat <- function(lonlat) {
  if (all.equal(length(lonlat), 2) & is.numeric(lonlat)) {
  ...
  } else if (all.equal(length(lonlat), 4) & is.numeric(lonlat)) {
    ...
    stop("You have entered an invalid request for 'lonlat'.")
  }
  return(identifier)
}
