...
station data that lacks radiation and format for use in APSIM by creating a
.met file using the R package, _APSIM_ (Fainges 2017).
...
proxy for potential incoming radiation, `SRAD`, we'll use `swv_dwn`,
...
```{r compare, eval=TRUE}
compare <- data.frame(kingsData$rad, swv_dwn$swv_dwn)
```
...
ggplot(compare, aes(x = kingsData.rad, y = swv_dwn.swv_dwn)) +
  ...
  cor(compare$kingsData.rad, compare$swv_dwn.swv_dwn)
```

The graph's 1:1 line shows a pretty good relationship between the two sets of
data as does the correlation.
...
```{r remove_rad, eval=TRUE, include=FALSE}
