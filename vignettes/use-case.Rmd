...
```{r load-libraries, message=FALSE, eval=TRUE}
library(APSIM)
library(nasapower)
library(ggplot2)
```
...
```{r get-data-execute, eval=TRUE, message=FALSE, include=FALSE}
...
The graph shows a strong relationship between the two sets of data as does the
...
```{r remove-rad, eval=TRUE, include=FALSE}
kings_nasa <- kingsData[, -6]
```

```{r create-met, eval=TRUE}
