...
### Example fetching inter-annual data for an area

Fetch inter annual solar cooking parameters for south east Queensland region.
...
Fetch global "AG"" climatology for temperature, precipitation and relative
humidity.
...
### Example converting regional data to a _raster_ object
...
### Example converting global climatology to a _raster_ object

Converting global climatology to a `raster::brick` is as simple as querying
and then converting the resulting `tibble` to a _raster_ object. Illustrated
here using the `climatology_ag` object previously created.
