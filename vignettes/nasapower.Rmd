...
_nasapower_ only provides one function, `get_cell()`, which will download
...
To start using _nasapower, `get_cells()` offers default values, all that is
necessary is to supply the latitude and longitude values. Weather variables will
default to T2M, T2MN, T2MX and RH2M (see below for more weather variables and
...
This function is useful for retreiving whole states, countries or global
...
Using _ggplot_, plot the downloaded data.
