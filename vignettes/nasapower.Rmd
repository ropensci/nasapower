...
#> Error in lapply(daily_region_ag, rasterFromXYZ, crs = "+proj=eqc +lat_ts=0 +lat_0=0 +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"): object 'daily_region_ag' not found
...
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'stack': object 'raster_list' not found
...
Converting global climatology to a `raster` objects is as simple as querying and then converting the resulting `tibble` to a _raster_ object using `terra::rast(type = "xyz")`<sup>*</sup>.
...
RH2M_ann <- rasterFromXYZ(climatology_list$RH2M[c(1:2, 15)],
                          crs = "+proj=eqc +lat_ts=0 +lat_0=0 +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs")
#> Error in rasterFromXYZ(climatology_list$RH2M[c(1:2, 15)], crs = "+proj=eqc +lat_ts=0 +lat_0=0 +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"): object 'climatology_list' not found
...
```r
library("terra")
T2M_brick <- rast(subset(climatology_ag, PARAMETER == "T2M")[-3], type = "xyz")
#> Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'rast': error in evaluating the argument 'x' in selecting a method for function 'subset': object 'climatology_ag' not found
