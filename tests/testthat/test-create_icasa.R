...
test_that(".icasa_checks creates a proper file path for `file_out`", {
  ...
  expect_equal(out[[8]],
               "! SRAD     All Sky Insolation Incident on a Horizontal Surface (MJ/m^2/day) ")
  expect_equal(out[[9]],
               "! Mean elevation for source NASA/POWER data tile (0.5 x 0.5 grid cell) = 434.55m")
